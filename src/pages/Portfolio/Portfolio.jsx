...
const PortfolioPage = () => {
  ...
  const PortfolioPreview = ({ data }) => (
    <div className={`p-6 ${data.theme === 'dark' ? 'bg-white text-white' : 'bg-white text-gray-700'}`}>
      <h1 className="text-3xl font-bold mb-2">{data.name}</h1>
      <h2 className="text-xl mb-4">{data.role}</h2>
      <p className="mb-6">{data.about}</p>
      <h3 className="text-2xl font-semibold mb-4">Projects</h3>
      <ul>
        {Object.entries(data.projects).map(([id, isSelected]) => (
          isSelected && <li key={id} className="mb-2">Project {id}</li>
        ))}
      </ul>
    </div>
  );
  ...
  return (
    <div className="flex flex-col md:flex-row gap-5 justify-center py-5 px-5">
      <section className="w-full md:w-1/2">
        <h1 className="pb-3 text-lg font-bold gap-3">Create Your Portfolio Here!</h1>
